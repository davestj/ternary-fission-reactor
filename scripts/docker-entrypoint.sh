...
# Date: July 31, 2025
# Title: Docker Container Entrypoint Script for Ternary Fission Energy Emulation - FIXED
...
# 2025-07-31: FIXED directory writability check to skip read-only mounted configs directory
#
# Carry-over Context:
...
# - Fixed configs directory check since it's mounted read-only from host
...
# We validate that required directories exist and are accessible
validate_directories() {
    log_debug "Validating application directories..."

    # We define directories that need to be writable vs read-only
    local writable_dirs=("${DATA_DIR}" "${RESULTS_DIR}" "${LOGS_DIR}")
    local readonly_dirs=("${CONFIG_DIR}")

    # We check writable directories
    ...
    # We check read-only directories (configs mounted from host)
    for dir in "${readonly_dirs[@]}"; do
        ...
        log_debug "Read-only directory validated: ${dir}"
    done
...
check_system_health() {
    ...
    available_disk=$(df "${DATA_DIR}" | awk 'NR==2{print $4}' 2>/dev/null || echo "unknown")

    if [[ "${available_disk}" != "unknown" ]] && [[ ${available_disk} -lt 1048576 ]]; then  # 1GB in KB
    ...
    local required_binaries=("${BIN_DIR}/api-server" "${BIN_DIR}/ternary-fission")
...
wait_for_services() {
    ...
    if [[ "${PROMETHEUS_ENABLED:-true}" == "true" ]]; then
        ...
        for i in {1..10}; do
            ...
            if [[ $i -eq 10 ]]; then
                log_warn "Prometheus not available after 10 attempts, continuing anyway..."
            else
                log_debug "Waiting for Prometheus... (attempt $i/10)"
                sleep 2
            fi
        done
    fi
...
start_api_server() {
    ...
    if [[ "${ready}" == "true" ]]; then
        ...
        log_warn "API server may not be fully ready, but continuing..."
    fi
...
start_simulation_engine() {
    ...
    "${BIN_DIR}/ternary-fission" "${engine_args[@]}" &
    local engine_pid=$!
...
start_daemon_mode() {
    ...
    "${BIN_DIR}/ternary-fission" --daemon --config="${CONFIG_FILE}" --rate="${EVENTS_PER_SECOND:-5.0}" &
    local engine_pid=$!
...
run_benchmark() {
    ...
    "${BIN_DIR}/ternary-fission" "${benchmark_args[@]}"
...
start_interactive_shell() {
    ...
    exec "${BIN_DIR}/ternary-fission" --interactive --config="${CONFIG_FILE}"
}
